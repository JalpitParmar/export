...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('Product updated successfully!'); window.location='adminproduct.php';</script>";
    } else {
        // Display the actual MySQL error for debugging
        echo "<script>alert('Error: Could not update product. " . mysqli_error($conn) . "');</script>";
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Update Product | Global Taste Exports</title>
  ...
  <style>
    ...
    * {
    ...
    /* NEW: Style for new image indicator */
    .new-image-indicator {
      margin-top: 5px;
      color: var(--primary-color);
      font-size: 0.8rem;
      ...
      <div class="add-product-content">
        ...
        <div class="form-container">
          ...
          <form id="productForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-section">
              ...
              <div class="form-group">
                ...
                <select class="form-control" id="productCategory" name="productCategory" required>
                  <option value="">Select Category</option>
                    <option value="a2-cow-ghee" <?php echo ($product['category'] == 'a2-cow-ghee') ? 'selected' : ''; ?>>A2 Cow Ghee</option>
                    <option value="beverages" <?php echo ($product['category'] == 'beverages') ? 'selected' : ''; ?>>Beverages</option>
                    <option value="cooking-paste-chutney" <?php echo ($product['category'] == 'cooking-paste-chutney') ? 'selected' : ''; ?>>Cooking Paste & Chutney</option>
                    <option value="dry-bhakhri" <?php echo ($product['category'] == 'dry-bhakhri') ? 'selected' : ''; ?>>Dry Bhakhri</option>
                    <option value="indian-sweets" <?php echo ($product['category'] == 'indian-sweets') ? 'selected' : ''; ?>>Indian Sweets</option>
                    <option value="jam" <?php echo ($product['category'] == 'jam') ? 'selected' : ''; ?>>Jam</option>
                    <option value="namkeen" <?php echo ($product['category'] == 'namkeen') ? 'selected' : ''; ?>>Namkeen</option>
                    <option value="pickles" <?php echo ($product['category'] == 'pickles') ? 'selected' : ''; ?>>Pickles</option>
                    <option value="ready-to-eat" <?php echo ($product['category'] == 'ready-to-eat') ? 'selected' : ''; ?>>Ready to Eat</option>
                    <option value="sauces" <?php echo ($product['category'] == 'sauces') ? 'selected' : ''; ?>>Sauces</option>
                    <option value="spices-powder" <?php echo ($product['category'] == 'spices-powder') ? 'selected' : ''; ?>>Spices Powder</option>
                    <option value="whole-spices" <?php echo ($product['category'] == 'whole-spices') ? 'selected' : ''; ?>>Whole Spices</option>
                </select>
              </div>
              ...
              <div class="form-group">
                ...
                <textarea class="form-control" id="keyFeatures" name="keyFeatures" rows="3" required><?php echo htmlspecialchars($product['key_features']); ?></textarea>
              </div>
            </div>
            ...
            <div class="form-section">
              ...
              <div class="image-upload-container">
                <div class="current-image-wrapper">
                    ...
                    <img src="../<?php echo htmlspecialchars($product['image_path']); ?>" alt="Current Product Image" id="currentProductImage">
                    <div class="new-image-indicator" id="newImageIndicator">
                        <i class="fas fa-info-circle"></i> New image selected (not saved yet)
                    </div>
                </div>
          ...
          <div class="form-footer">
            ...
            <button type="submit" form="productForm" class="btn btn-primary" id="updateProductBtn">
  ...
  <script>
    ...
    const ImageUpload = {
      elements: {
        ...
        currentImage: document.getElementById('currentProductImage'),
        fileInput: document.getElementById('productImage'),
        imageName: document.getElementById('imageName'),
        newImageIndicator: document.getElementById('newImageIndicator'),
        originalImageSrc: null
      },

      init() {
        // Store the original image source
        this.elements.originalImageSrc = this.elements.currentImage.src;
        this.bindEvents();
      },

      bindEvents() {
        // Clicking the upload box or the current image opens the file dialog
        this.elements.imageUploadBox.addEventListener('click', () => {
          this.elements.fileInput.click();
        });

        this.elements.currentImage.addEventListener('click', () => {
          this.elements.fileInput.click();
        });
      ...
      handleImageSelect(e) {
        ...
        if (input.files && input.files[0]) {
          ...
          reader.onload = (e) => {
            // Update the current image with the new preview
            this.elements.currentImage.src = e.target.result;
            
            // Show the new image indicator
            this.elements.newImageIndicator.style.display = 'block';
            
            // Also update the upload box to show the preview
          ...
          this.elements.currentImage.src = this.elements.originalImageSrc;
          this.elements.newImageIndicator.style.display = 'none';
    ...
    const PacketSizes = {
      ...
      idCounter: 0,
      existingPacketSizes: <?php echo json_encode(explode(',', $product['packet_sizes'])); ?>,

      init() {
        this.bindEvents();
        // Initialize with existing packet sizes
        this.initializePacketSizes();
      },
      ...
      initializePacketSizes() {
        // Add existing packet sizes
        if (this.existingPacketSizes.length > 0) {
          this.existingPacketSizes.forEach(size => {
            if (size.trim() !== '') {
              this.addPacketSize(size.trim(), true);
            }
          });
        } else {
          // Add one empty packet size if none exist
          this.addPacketSize('', false);
        }
      },

      addPacketSize(value = '', isChecked = false) {
        ...
        packetSizeItem.innerHTML = `
          <input type="checkbox" id="${id}" name="packet[]" value="${value}" ${isChecked ? 'checked' : ''}>
